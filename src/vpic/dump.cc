...
#define fpp(x, y, z) f[VOXEL(x, y, z, grid->nx, grid->ny, grid->nz)]
    ...
    field_global_size[0] = (grid->nx * px);
    field_global_size[1] = (grid->ny * py);
    field_global_size[2] = (grid->nz * pz);
...
#define DUMP_HYDRO_TO_HDF5(DSET_NAME, ATTRIBUTE_NAME, ELEMENT_TYPE)                                               \
    ...
    hydro_global_size[0] = (grid->nx * px);
    hydro_global_size[1] = (grid->ny * py);
    hydro_global_size[2] = (grid->nz * pz);
...
(iz) = _iz;                                                         \
    ...
    for (int i = 0; i < numparticles; i++)
    {
        ...
        UNVOXEL(mpi_rank, rx, ry, rz, px, py, pz);
